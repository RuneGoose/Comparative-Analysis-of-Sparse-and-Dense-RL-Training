...
if __name__ == "__main__":
    ...
    agent = Agent()
    checkpoint_path = f"{folder_name}/{folder_name}_ppo_net_params.pkl"
    epoch_tracker = f"{folder_name}/epoch_checkpoint.txt"
    start_epoch = 0
    ...
    if os.path.exists(epoch_tracker):
        with open(epoch_tracker, "r") as ef:
            start_epoch = int(ef.read().strip()) + 1
    ...
    running_score = 0
    for i_ep in tqdm(range(start_epoch, start_epoch + epochs)):
        score = 0
        state = env.reset()
        for _ in range(1000):
        ...
        with open(epoch_tracker, "w") as ef:
            ef.write(str(i_ep))
